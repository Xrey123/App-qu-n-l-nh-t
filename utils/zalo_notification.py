...
class ZaloNotifier:
    ...
    def send_balance_notification(
        self, user_phone: str, username: str, balance: float, balance_type: str = "nợ"
    ) -> bool:
        """
...
"""
    ...
    def _send_text_message(self, phone: str, message: str) -> bool:
        ...
        payload = {
            "recipient": {"user_id": phone},  # hoặc "phone_number" tùy API version
    ...
    def send_bulk_balance_notifications(self, users_data: list) -> dict:
        ...
        for user in users_data:
            ...
            success = self.send_balance_notification(
                user_phone=phone, username=username, balance=balance
            )
            ...
            results["details"].append(
                {"phone": phone, "username": username, "success": success}
            )

        logger.info(
            f"Bulk notification: {results['success']} success, {results['failed']} failed"
        )
        return results
...
def notify_user_balance(
    user_id: int, username: str, balance: float, phone: str = None, method: str = "oa"
) -> bool:
    """
